...
class Controller(object):
    ...
    def _parse(self, input):
        """Takes the ui frame string and returns a list of variables used within the strings"""
        result = []
        count = 0
    ...
    def _mkdict(self):
        """Takes the list generated by _parse and creates a dictionary that fills in attributes
        from the player object or leaves a blank string if the variables don't match anything in player.contents"""
    ...
    def display(self, text, len=75, clear=50):
        ...
        # This has to update player stats by rereading the whole dictionary every time :/
        self._mkdict()
        tools.clear(clear)
        ...
        while working:
            ...
            # find a line break (if any) including up to the very possible end, hence the +1
            # anything beyond is certain to be a part of the next line
            if '\n' in working[:len+1]:
                find = working.find('\n')
            else:
                find = len
            ...
            # set the line# with the string that should be under the max char. limit
            if self.fill.get(fillkey) == "":
                self.fill[fillkey] = working[:find]
            else:
                # if there is more text than available lines, we need another box
                cont = True
                break
            
            working = working[find:].lstrip('\n') ### QUESTION, when break is called, it still removes the last used statement, HOW?
            count += 1
